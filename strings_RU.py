...

{
    ...
    'admin_required' : 'Вы должны иметь права администратора чтоб выполнять эту команду.',

    'help' : '''Пожалуйста, посетите: https://reminder-bot.com/help?lang=EN''',
...
Убедитесь что таймер не превышает 50 лет вперёд.''',
        ...
        'invalid_chars' : '''Сообщение слишком длинное! (максимум 150 символов, ваше сообщение содержит {} символов). Используйте `{prefix}donate` для увеличения до 1900 символов ($5).''',

        'invalid_chars_2000' : '''Максимальное количество символов для сообщений в дискорд - 2000. Извините, использовать больше символов технически невозможно.''',

        'no_perms' : '''Вы должны иметь права на редактирование сообщений или роль которой дозволено использовать бота. Пожалуйста, обратитесь к администратору и расскажите ему про `{prefix}restrict` команду, которая позволит исправить эту проблему.''',

        'success' : '''Новое уведомление будет отослано <{}{}> через {} секунд. Если вы хотите отменить, напишите `$del`.''',
    },
...
Usage:
    ...
    ''',
        ...
        'success' : '''Новое напоминание зарегистрировано {} через {} секунд. Если вы хотите его удалить, напишите `$del`.''',

        'bad_time' : '''Время указано неверно. Напишите время более чётко, для примера `16th of july` или `in 20 minutes`''',
...
Убедитесь что интервал установлен не на 50 лет вперёд''',
        ...
        '8_seconds' : '''Убедитесь что интервал настроен не чаще 8 секунд.''',
        ...
        'success' : '''Новый интервал добавлен <{}{}> через {} секунд . Теперь вы не можете редактировать сообщение, можете его удалить.''',

        'removed' : '''Теперь на вашем сервере нет людей поддержавших бота, интервалы отключены.'''
    ...
    'todo' : {
        'server_only' : '''Используйте `{prefix}todo` для вашего персонального списка запланированного. `{prefix}todos` для списка запланированного сервера.''',

        'add' : '''*Используйте `{prefix}{command} <message>` добавить заметку, или напишите `{prefix}{command} help` для получения полной информации!*''',

        'too_long' : '''Извините, но максимальная длинна заметки составляет 80 символов. Держите это в уме :)''',

        'too_long2' : '''Извините, но список запланированного не может превышать 800 символов.''',
