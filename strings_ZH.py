...

{
    ...
    'help_raw' : [
        ['''提醒事项指令''', {
            '$natural' : '一种设置提醒的更简单的方式。请输入`$natural`来获取更多信息。',

            '$del' : '删除服务器上的提醒事项或周期重复提醒。如果是私信提醒，请给bot私信发送这个指令。',

            '$remind [用户/频道] <提醒时间> <内容>' : '请优先考虑使用`$natural`而不是用这个指令。设置一个提醒。将时间设置成 [数字][s/m/h/d] 的格式，比如 10s 即10秒， 2s10m 即10分钟2秒。也可用 `日`/`月`/`年`-`时`:`分`:`秒` 的格式设置精确时间。',

            '$interval [用户/频道] <提醒时间> <重复周期> <内容>' : '请优先考虑使用`$natural`而不是用这个指令。设置一个周期重复提醒，给出的`内容`将从`提醒时间`开始，每隔`重复周期`被重复提醒一次。将时间设置成上述格式。例如`$interval 0s 20m Hello World!`就会每20分钟向该频道发送一次`Hello World!`。',
        ...
        ['''其他指令''', {
            ...
            '$clock' : '在频道列表中显示一个实时更新的时钟。仅限管理员使用。',
